...
def build_prithvi_task(freeze_backbone=True, backbone_pretrained=True):
...
class WeatherConditionedWildfire(LightningModule):
    ...
    """
    def __init__(self, weather_dim=5, freeze_backbone=True, lr=1e-4, backbone_pretrained=True):
        super().__init__()
        self.lr = lr
        self.save_hyperparameters()

        # Base Prithvi task
        self.prithvi_task = build_prithvi_task(freeze_backbone=freeze_backbone, backbone_pretrained=backbone_pretrained)
